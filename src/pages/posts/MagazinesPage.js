...
export default () => {
  ...
  const columns = [
    ...
    {
      label: t("POSTS.MAGAZINES.DATE"),
      field: "date",
      sort: "asc",
    },
    {
      label: t("POSTS.MAGAZINES.TITLE"),
      field: "date",
      sort: "asc",
    },
    {
      label: t("POSTS.MAGAZINES.DESCRIPTION"),
      field: "date",
      sort: "asc",
    },
    {
  ...
  const makeButtons = (id, number) => {
    return (
      <Fragment>
        <Link to={`${routes.posts.addMagazine}/${id}`}><MDBBtn tag="a" size="sm" color="indigo" className="mr-2" floating><MDBIcon icon="edit"/></MDBBtn></Link>
  ...
  const deleteItem = id => {
    Service.deleteMagazine({id: modal.deleteId, page: currentPage})
      .then(res => {
        if (res.result === SUCCESS) {
          for (let row of res.data) {
            row["button"] = makeButtons(row.id, row.number);
  ...
  const handlePageChange = page => {
    history.push(`${routes.posts.magazines}/${page}`);
  };
  ...
  return (
    <Fragment>
      <Helmet>
        <title>{t("NAVBAR.POSTS.MAGAZINES")} - {t("SITE_NAME")}</title>
      </Helmet>
      <MDBBreadcrumb>
        <MDBBreadcrumbItem>{t("NAVBAR.POSTS.POSTS")}</MDBBreadcrumbItem>
        <MDBBreadcrumbItem active>{t("NAVBAR.POSTS.MAGAZINES")}</MDBBreadcrumbItem>
      </MDBBreadcrumb>
      {!!loading && <Loading/>}
      {!loading && <MDBRow>
        <MDBCol md={12}>
          <h3 className="mt-4 font-weight-bold text-center">{t("NAVBAR.POSTS.MAGAZINES")}</h3>
        </MDBCol>
        <MDBCol md={12}>
        ...
        <MDBCol md={12} className="text-left mt-3">
          <div className="full-width">
            <Link to={`${routes.posts.addMagazine}`}>
              <MDBBtn size="sm" color="primary">
                {t("POSTS.MAGAZINES.ADD_MAGAZINE")}
              </MDBBtn>
            </Link>
          </div>
        </MDBCol>
