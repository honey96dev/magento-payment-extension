...
export default () => {
  ...
  useEffect(e => {
    ...
    PostsService.list({page, allowed: 0})
      .then(res => {
  ...
  const handlePageChange = page => {
    history.push(`${routes.posts.denied}/${page}`);
  };
  ...
  return (
    <Fragment>
      <Helmet>
        <title>{t("NAVBAR.POSTS.DENIED")} - {t("SITE_NAME")}</title>
      </Helmet>
      <MDBBreadcrumb>
        <MDBBreadcrumbItem>{t('NAVBAR.POSTS.POSTS')}</MDBBreadcrumbItem>
        <MDBBreadcrumbItem active>{t('NAVBAR.POSTS.DENIED')}</MDBBreadcrumbItem>
      </MDBBreadcrumb>
      {!!loading && <Loading/>}
