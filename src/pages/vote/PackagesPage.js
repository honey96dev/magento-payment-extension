...
export default () => {
  ...
  const makeButtons = (id, number) => {
    return (
      <Fragment>
        <Link to={`${routes.vote.addPackage}/${page || 1}/${id}`}><MDBBtn tag="a" size="sm" color="indigo" floating><MDBIcon icon="edit"/></MDBBtn></Link>
      ...
      {!loading && !!items.length && <MDBRow>
        ...
        <MDBCol md={12} className="text-left mt-3">
          <div className="full-width">
            <Link to={`${routes.vote.addPackage}/${page || 1}`}>
